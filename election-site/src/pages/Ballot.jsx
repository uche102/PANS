...
const Ballot = () => {
  ...
  // State to track selected candidate IDs for each office
...
const handleSubmitVotes = async () => {
  ...
  const response = await fetch("http://localhost:8000/api/vote", {
    ...
    body: JSON.stringify({
      regNo: localStorage.getItem("voterRegNo") || "DEMO-USER-2026",
    ...
    if (response.ok) {
      ...
      // Handles "Already Voted" or other server errors
          ...
          <section key={section.office} className="office-section">
            ...
            <div className="candidates-grid">
              {section.candidates.map((candidate) => (
                <div
                  key={candidate.id}
                  className={`candidate-card ${selections[section.office] === candidate.id ? "selected" : ""}`}
